...
This is the file for analysis of the growth model in project/linkedModels/manuscriptAnalysis
Original name was 'MS_Analysis_3Spp_WB.Rmd'

Will take parts from this file for 'growingFish analysis.rmd'
...
ggplot( means %>% filter(year >= 2000, species != 'ats'), aes(year,meanLen, group= speciesGG)) +
  ...
  geom_text(data = lenSlopesTidy %>% filter(species == 'bkt', age<2), 
            aes(x=2000,y=225, label=paste(round(estimate,2),sig)),hjust = 0)+#, color = speciesGG)) +
  geom_text(data = lenSlopesTidy %>% filter(species == 'bnt', age<2), 
            aes(x=2000,y=203, label=paste(round(estimate,2),sig)),hjust = 0)+#, color = speciesGG)) +
#  geom_text(data = lenSlopesTidy %>% filter(species == 'ats', age<2), aes(x=2000,y=181, label=paste(round(estimate,2),sig)),hjust = 0)+#, color = speciesGG)) +
  ...
  geom_text(data = lenSlopesTidy %>% filter(species == 'bkt', age==2), 
            aes(x=2012.5,y=104, label=paste(round(estimate,2),sig)),hjust = 0)+#, color = speciesGG)) +
  geom_text(data = lenSlopesTidy %>% filter(species == 'bnt', age==2), 
            aes(x=2012.5,y=82, label=paste(round(estimate,2),sig)),hjust = 0)+#, color = speciesGG)) +
...
ggsave('figures/3Spp_WB/flow.png', width = 11, height = 7, units='in')
...
ggplot(meanEnv, aes(meanT,meanF) ) +
  geom_point( size = 3 ) +
  geom_smooth(method = 'lm', se=F) +
  labs( x = "Stream Temperature", y = "Stream flow (m^3/s)" ) +
  theme_publication() +
  facet_wrap(~seasonGG)
...
ggsave('./figures/3Spp_WB/predLensTempBkt.png', width = 11, height = 9, units = 'in')
...
ggplot(bktLoHi, aes(occ,lenMean, group = bkt)) +
  geom_line() +
  geom_segment(data = segment_dataBKT, aes(x=x,xend=xend,y=y,yend=yend), size = 2, color = 'black')

bntLoHi <- stats3Spp[['bkt']][[1]] %>% filter(iter == 1, bnt %in% c(-1.5,1.5), temp %in% 0, flow %in% 0, bkt %in% 0, ats %in% 0)
...
```{r models with occ, season,isYOY for looking at > 2-way interactions - not used for paper}
  datOccBKTAll <- grStats3Spp[['bkt']][[1]] %>% filter(iter == 1, temp %in% indepVarVals, flow %in% indepVarVals, bkt %in% indepVarVals, bnt %in% indepVarVals, ats %in% indepVarVals)
  lmAllBKT <- lm( grMean ~ (temp + flow + temp * flow + temp^2 + flow^2 + bkt + bnt + ats + bnt * bkt)* occ, data = datOccBKTAll) 
summary(lmAllBKT)
anova(lmAllBKT)

  lmAllBKTSeason <- lm( grMean ~ (temp + flow + temp * flow + temp^2 + flow^2 + bkt + bnt + ats + bnt * bkt)* season, data = datOccBKTAll) 
summary(lmAllBKTSeason)
anova(lmAllBKTSeason)

datOccBKTAll$isYOY <- ifelse(datOccBKTAll$occ %in% 1:3, 1, 0)
  lmAllBKTSeasonYOY <- lm( grMean ~ (temp + flow + temp * flow + temp^2 + flow^2 + bkt + bnt + ats + bnt * bkt)* season * isYOY, data = datOccBKTAll) 
summary(lmAllBKTSeasonYOY)
anova(lmAllBKTSeasonYOY)


# 2-way max
  lmAllBKT2 <- lm( grMean ~ (temp + flow +  temp^2 + flow^2 + bkt + bnt + ats)* occ + temp*flow, data = datOccBKTAll) 
summary(lmAllBKT2)
anova(lmAllBKT2)
relImp2WayBKT <- calc.relimp(lmAllBKT2, type = c("lmg"), rela = TRUE)

  datOccBNTAll <- grStats3Spp[['bnt']][[1]] %>% filter(iter == 1, temp %in% indepVarVals, flow %in% indepVarVals, bkt %in% indepVarVals, bnt %in% indepVarVals, ats %in% indepVarVals)  
  lmAllBNT2 <- lm( grMean ~ (temp + flow +  temp^2 + flow^2 + bkt + bnt + ats)* occ + temp*flow, data = datOccBNTAll) 
summary(lmAllBNT2)
anova(lmAllBNT2)
relImp2WayBNT <- calc.relimp(lmAllBNT2, type = c("lmg"), rela = TRUE)
save(relImp2WayBKT, relImp2WayBNT, file = './data/out/datOccAllWtemp_flow.RData')

relImpOcc <- bind_cols(as.data.frame(relImp2WayBKT$lmg) %>% rownames_to_column(), bnt = as.data.frame(relImp2WayBNT$lmg)) %>% gather('spp', 'value', 2:3)

relImpOcc$rownameGG <- factor(relImpOcc$rowname, 
                              levels = c('occ','temp','temp:occ','flow','flow:occ','temp:flow','bkt','bkt:occ','bnt','bnt:occ','ats','ats:occ'),
                              labels = c('Time','Temp','Temp:Time','Flow','Flow:Time','Temp:Flow','Bkt','Bkt:Time','Bnt','Bnt:Time','Ats','Ats:Time'), ordered = TRUE)

relImpOccTmp <- relImpOcc %>%
  filter(rowname != 'occ') %>%
  group_by(spp) %>%
  summarise(sum = sum(value))
relImpOcc <- left_join(relImpOcc, relImpOccTmp) %>% mutate(valueAdj = value/sum)

#relImpOcc <- rename(relImpOcc, var=rowname, bkt = relImp2WayBKT$lmg, bnt = relImp2WayBNT$lmg)
ggplot(relImpOcc%>%filter(rowname %in% c('temp','flow','temp:flow','bkt','bnt','ats')), aes(rownameGG, valueAdj, group = spp, color = spp)) +
  geom_point(size = 4) + 
  geom_line() +
  scale_x_discrete("Variable name") +
  ...
  labs(color='Species') +
  scale_color_grey(start = 0.15, end = 0.75, labels = c('Brook Trout', 'Brown Trout'))

ggsave('./figures/relImpOcc.png', width = 11, height = 9, units = 'in')

ggplot(relImpOcc%>%filter(rowname != 'occ'), aes(rownameGG, valueAdj, group = spp, color = spp)) +
  geom_point(size = 4) + 
  geom_line() +
...
ggsave('./figures/relImpOccAll.png', width = 11, height = 9, units = 'in')
...
```{r RI across occ - use for Spain talk and maybe Growth paper?}
...
#for( occSelected in 1:(simData$nOcc - 1)){
  ...
  for(spp1 in spp){
    ...
    datAgeGroupSeason <- grStats3Spp[[spp1]][[1]] %>%
         dplyr:::mutate(ageGroup = ifelse(occ %in% 1:3, 0, 1),
  ...
  rI <- cbind(data.frame(bktRI = relImpAll[['bkt']]$lmg), 
        ...
        rownames_to_column() %>% 
        gather(key=rowname) %>% 
        dplyr:::mutate(var = rep(1:length(relImpAll[['bkt']]$lmg),length(spp)),
...
#save(rIAllMinusQ, file = 'data/out/rIAllMinusQ.RData')

# graph of prop explained over time
summ1b <- summ1a %>% mutate(ageSeason = paste0(ageGroup,season)) 
summ1b$ageSeason2 <- ifelse(summ1b$ageSeason == '01', "05", summ1b$ageSeason )
  
ggplot(summ1b, aes(ageSeason2, prop, group = rowname, color = rowname)) +
  geom_point(size = 2) +
  geom_line() +
 # scale_x_discrete(labels = c('0/3','0/4','1/1','1/2','1/3','1/4')) +
  labs( x = "Age Season", y = "Proportion density-independent" ) +
  theme_publication(base_size = 24) +
  theme(legend.title = element_text("Species"),
        legend.position = "bottom",
        legend.direction = "horizontal",
