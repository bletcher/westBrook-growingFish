...
---
  ...
  See 'linearGR_ModelsMap_BKT.R' for model selection code for the growth model

Manuscript is saved on Teams in 'Keith_Ben MSs'. Open file there and edit on desktop.
...
d2 %>% group_by(riverOrderedGG,seasonGG,year) %>%
  ...
  geom_errorbar( aes(ymin = meanT - sdT, ymax = meanT + sdT, width = 0.33 ) ) +
  labs( x = "Year", y = "Stream temperature (C)" ) +
  geom_text(data = tempSlopesTidy %>% filter(riverOrderedGG == "West Brook"),aes(x=2000.5,y=c(11, 17.5, 5, 2.7), label = paste(round(estimate,2),sig)), size = 4.5) +
  scale_color_grey(start = 0, end = 0.7) +
  theme_publication(base_size = 24)

ggsave('figures/tempNoFacet.png', width = 9, height = 7, units='in')

save(tempSlopesTidy,file = 'D:/projects/westBrook-growingFish/data/out/tempSlopesTidy.RData')
```

```{r output env data for Observable}

envOut <- d2 %>% group_by(riverOrderedGG,seasonGG,year) %>%
  filter(riverOrderedGG == "West Brook" ) %>%
  summarize(meanF = mean(meanFlow, na.rm=T),
            sdF = sd(meanFlow, na.rm=T),
...
write.csv(envOut, row.names = FALSE, 'D:/projects/westBrook-growingFish/data/out/forObservable/envMeansOut.csv')

# slopes and intercepts for env model
tempForObs <- tempMods %>%
  mutate(tidy = map(model,broom::tidy)) %>%
  dplyr::select(-data,-model) %>%
  unnest() %>%
  filter(riverOrderedGG == "West Brook") %>%
  select(seasonGG, term, estimate) %>%
  mutate(var = "temp")
...
write.csv(bind_rows(tempForObs, flowForObs), row.names = FALSE, 'D:/projects/westBrook-growingFish/data/out/forObservable/envSlopesOut.csv')
```
...
reconnect() 
conDplyr # show tables

temp0 <- as.data.frame(tbl(conDplyr, "data_daily_temperature")) 
temp <-  temp0 %>%
  filter(river == "west brook") %>%
  mutate(doy = yday(as.POSIXct(date)),
         year = year(as.POSIXct(date)),
         season = case_when(doy <= 36 ~ 4,
                            doy > 36 & doy <= 126 ~ 1,
                            doy > 126 & doy <= 216 ~ 2,
                            doy > 216 & doy <= 296 ~ 3,
                            doy > 296 ~ 4)
  )
temp$seasonGG <- factor(temp$season, labels = c("Spring","Summer","Autumn","Winter"), ordered = T)

ggplot(temp, aes(doy,daily_mean_temp, group = seasonGG, color = seasonGG)) +
  geom_smooth(method = 'lm', se=F) +
  #geom_point(aes(shape = factor(seasonGG), size = 2)) +
  geom_point(size = 3) +
  #geom_line() +
  ...
  scale_color_grey(start = 0, end = 0.7) +
  theme_publication(base_size = 24) +
  facet_wrap(seasonGG ~ year)


tempByYear <- temp %>%
  group_by(year, seasonGG) %>%
  summarize(meanT = mean(daily_mean_temp, na.rm=T),
            sdT = sd(daily_mean_temp, na.rm=T)) %>%
  ungroup()

ggplot(tempByYear, aes(year,meanT, group = seasonGG, color = seasonGG)) +
  geom_smooth(method = 'lm', se=F) +
  #geom_point(aes(shape = factor(seasonGG), size = 2)) +
  geom_point(size = 3) +
  #geom_line() +
...
```{r n out for Observable}

nOut <- allFishBySpeciesYOY %>% filter(riverOrderedGG == "West Brook", year >=2000)

write.csv(nOut, row.names = FALSE, 'D:/projects/westBrook-growingFish/data/out/forObservable/nMeansOut.csv')

nSlopesTidy2 <- nSlopes %>%
  mutate(tidy = map(model,broom::tidy)) %>%
  dplyr::select(-data,-model) %>%
  unnest() %>%
  #filter(riverOrderedGG == "West Brook") %>%
...
nSlopesTidy2$seasonGG <- factor(nSlopesTidy2$season, labels = c("Spring","Summer","Autumn","Winter"), ordered = T)
nSlopesTidy2$speciesGG <- factor(nSlopesTidy2$species, levels = c('bkt','bnt','ats'), labels = c("Brook Trout", "Brown Trout", "Atlantic Salmon"), ordered = T)

write.csv(nSlopesTidy2 %>% select(species, season, seasonGG, term, estimate), row.names = FALSE, 'D:/projects/westBrook-growingFish/data/out/forObservable/nSlopesOut.csv')
...
ggplot(means %>% filter(year >= 2000, species != 'ats'), aes(year,meanLen, group= speciesGG)) +
  ...
  geom_text(data = lenSlopesTidy %>% filter(species == 'bkt', age<2, term == "year"), 
            aes(x=2000,y=225, label=paste(round(estimate,2),sig)),hjust = 0, size = 6)+#, color = speciesGG)) +
  geom_text(data = lenSlopesTidy %>% filter(species == 'bnt', age<2, term == "year"), 
...
means$yearCentered <- means$year - 2008

modMeans1 = lm(meanLen ~ yearCentered * species, data = means %>% filter(year >= 2000, species != 'ats', ageInSamples == 1))
modMeans2 = lm(meanLen ~ yearCentered * factor(ats01) * species, data = means %>% filter(year >= 2000, species != 'ats', ageInSamples == 1))
modMeans3 = lm(meanLen ~ factor(ats01) * species, data = means %>% filter(year >= 2000, species != 'ats', ageInSamples == 1))
modMeans4 = lm(meanLen ~ factor(ats01) * yearCentered, data = means %>% filter(year >= 2000, species != 'ats', ageInSamples == 1))
AIC(modMeans1, modMeans2, modMeans3, modMeans4) %>% arrange(AIC)
...
meansOut <- means %>% filter(year >= 2000)

write.csv(meansOut, row.names = FALSE, 'D:/projects/westBrook-growingFish/data/out/forObservable/meanLengthsOut.csv')

# slopes and intercepts for size model
write.csv(lenSlopesTidy %>% select(species, ageInSamples, season, age, term, estimate), row.names = FALSE, 'D:/projects/westBrook-growingFish/data/out/forObservable/lenSlopesOut.csv')

# overall means
# input directly into Observable in the meanLengthDataFromR variable
seasonalMeanLengths <- cd %>% mutate(age = year - cohort ) %>%
  filter(riverOrdered == 'west brook', ageInSamples %in% 1:10, age < 3, year < 2016) %>%
  group_by(species) %>%
  summarize(meanLen = mean(observedLength,na.rm = TRUE),
            sdLen = sd(observedLength,na.rm = TRUE),
...
seasonalMeanSampleInterval <- cd %>% mutate(age = year - cohort ) %>%
  filter(riverOrdered == 'west brook', ageInSamples %in% 1:10, age < 3, year < 2016) %>%
  group_by(species, season) %>%
  summarize(meanSampleInterval = mean(sampleInterval,na.rm = TRUE)) %>%
  arrange(season)

sampleIntervals <- cd %>% mutate(age = year - cohort ) %>%
  filter(riverOrdered == 'west brook', ageInSamples %in% 1:10, age < 3, year < 2016, species != "ats") %>%
  mutate(seasonGG = factor(season, levels = 1:4, labels = c("Spring","Summer","Autumn","Winter"), ordered = T)) %>%
  group_by(year, seasonGG) %>%
  summarize(meanSampleInterval = mean(sampleInterval,na.rm = TRUE)) %>%
  arrange(year,seasonGG)
  
write.csv(sampleIntervals, row.names = FALSE, 'D:/projects/westBrook-growingFish/data/out/forObservable/sampleIntervals.csv')

```


```{r size-dep imm or mort?}
...
mod5 <- glm(notLastObs*1 ~ observedLengthStd * species, family = 'binomial', data = cd01Mod)
...
#prob of last obs
pred$pred1Minus <- 1 - pred$pred

ggplot(pred %>% filter(cohortStd %in% 0, age < 3), aes(observedLengthStd, pred1Minus, color = speciesGG)) +
  geom_point(size = 2) +
  scale_x_continuous("Standardized length") +
  scale_y_continuous(lim = c(0,1),"Probability of last observation") +
  theme_publication(base_size = 28) +
  theme(legend.title = element_text("Species"),
        legend.position = "bottom",
        legend.direction = "horizontal"
...
ggsave('./figures/pLoss.png', width = 11, height = 11, units='in')
...
modi1a <- glm(firstObs*1 ~ (observedLengthStd + cohortStd + species + factor(ageInSamples))^2, family = 'binomial', data = cd01Mod)
...
AIC(modi1,modi1a,modi2,modi3,modi4,modi5) %>% rownames_to_column() %>% arrange(AIC)
...
```{r predicted lengths}
# datafiles created in 'linkedModel_growthMS_Analysis_3spp_WB.Rmd'
# contains dataframes: stats3Spp, grStats3Spp, elapsed, simData

load('D:/projects/linkedModels/data/out/projectionOut_N100.RData')

# bkt temp
stats3Spp[['bkt']][[1]]$iter_temp <- paste(stats3Spp[['bkt']][[1]]$iter,stats3Spp[['bkt']][[1]]$temp,sep='_')

tempLoHiBKT <- stats3Spp[['bkt']][[1]] %>% filter( temp %in% c(-1.5,1.5), flow %in% 0, bkt %in% 0, bnt %in% 0, ats %in% 0)

segment_dataBKT = data.frame(
    x = seq(1,10),
    y = c(69,79,78,105,119,130,121,146,158,165), 
    xend = seq(1,10),
    yend = c(77,84,95,131,145,142,160,180,190,190)
)

ggplot(tempLoHiBKT, aes(occ,lenMean)) +
  geom_line(aes(color = factor(temp), group = iter_temp), alpha = 0.25) +
  geom_segment(data = segment_dataBKT, aes(x=x,xend=xend,y=y,yend=yend), size = 2, color = 'black') +
  theme_publication(base_size = 24) +
  scale_x_discrete("Age/season", label = c('0/3','0/4','1/1','1/2','1/3','1/4','2/1','2/2','2/3','2/4')) +
  scale_y_continuous("Predicted length (mm)") +
  scale_color_manual(values = c('lightblue','red')) +
  labs(color='Standardized temperature')

ggsave('./figures/predLensTempBkt.png', width = 11, height = 9, units = 'in')

# bkt flow
stats3Spp[['bkt']][[1]]$iter_flow <- paste(stats3Spp[['bkt']][[1]]$iter,stats3Spp[['bkt']][[1]]$flow, sep='_')

flowLoHiBKT <- stats3Spp[['bkt']][[1]] %>% filter( flow %in% c(-1.5,1.5), temp %in% 0, bkt %in% 0, bnt %in% 0, ats %in% 0)

ggplot(flowLoHiBKT, aes(occ,lenMean)) +
  geom_line(aes(color = factor(flow), group = iter_flow), alpha = 0.25) +
  geom_segment(data = segment_dataBKT, aes(x=x,xend=xend,y=y,yend=yend), size = 2, color = 'black') +
  theme_publication(base_size = 24) +
  scale_x_discrete("Age/season", label = c('0/3','0/4','1/1','1/2','1/3','1/4','2/1','2/2','2/3','2/4')) +
  scale_y_continuous("Predicted length (mm)") +
  scale_color_manual(values = c('lightblue','red')) +
  labs(color='Standardized flow')

ggsave('./figures/predLensFlowBkt.png', width = 11, height = 9, units = 'in')


# bkt ATS
stats3Spp[['bkt']][[1]]$iter_ats <- paste(stats3Spp[['bkt']][[1]]$iter,stats3Spp[['bkt']][[1]]$ats, sep='_')

atsLoHiBKT <- stats3Spp[['bkt']][[1]] %>% filter( ats %in% c(-1.5,1.5), temp %in% 0, bkt %in% 0, bnt %in% 0, flow %in% 0)

ggplot(atsLoHiBKT, aes(occ,lenMean)) +
  geom_line(aes(color = factor(ats), group = iter_ats), alpha = 0.25) +
  geom_segment(data = segment_dataBKT, aes(x=x,xend=xend,y=y,yend=yend), size = 2, color = 'black') +
  theme_publication(base_size = 24) +
  scale_x_discrete("Age/season", label = c('0/3','0/4','1/1','1/2','1/3','1/4','2/1','2/2','2/3','2/4')) +
  scale_y_continuous("Predicted length (mm)") +
...
ggsave('./figures/predLensATSBkt.png', width = 11, height = 9, units = 'in')


# bkt temp AND ATS
stats3Spp[['bkt']][[1]]$iter_temp_ats <- paste(stats3Spp[['bkt']][[1]]$iter,stats3Spp[['bkt']][[1]]$temp,stats3Spp[['bkt']][[1]]$ats, sep='_')

stats3Spp[['bkt']][[1]]$temp_ats <- paste(stats3Spp[['bkt']][[1]]$temp,stats3Spp[['bkt']][[1]]$ats, sep='_')


tempAtsLoHiBKT <- stats3Spp[['bkt']][[1]] %>% filter(ats %in% c(-1.5,1.5), temp %in% c(-1.5,1.5), bkt %in% 0, bnt %in% 0, flow %in% 0)

ggplot(tempAtsLoHiBKT, aes(occ,lenMean)) +
  geom_line(aes(color = factor(temp), linetype=factor(ats), group = iter_temp_ats), alpha = 0.25) +
  geom_segment(data = segment_dataBKT, aes(x=x,xend=xend,y=y,yend=yend), size = 2, color = 'black') +
 # theme_publication(base_size = 24) +
  scale_x_discrete("Age/season", label = c('0/3','0/4','1/1','1/2','1/3','1/4','2/1','2/2','2/3','2/4')) +
  scale_y_continuous("Predicted length (mm)") +
...
ggsave('./figures/predLensTempATSBkt.png', width = 11, height = 9, units = 'in')


#####################
#bnt
stats3Spp[['bnt']][[1]]$iter_temp <- paste(stats3Spp[['bnt']][[1]]$iter,stats3Spp[['bnt']][[1]]$temp,sep='_')
tempLoHiBNT <- stats3Spp[['bnt']][[1]] %>% filter( temp %in% c(-1.5,1.5), flow %in% 0, bkt %in% 0, bnt %in% 0, ats %in% 0)

segment_dataBNT = data.frame(
    x = seq(1,10),
    y = c(67,75,73,103,120,135,126,165,174,185), 
    xend = seq(1,10),
    yend = c(79,84,97,135,157,158,175,200,226,240)
)

ggplot(tempLoHiBNT, aes(occ,lenMean)) +
  geom_line(aes(color = factor(temp), group = iter_temp), alpha = 0.25) +
  geom_segment(data = segment_dataBNT, aes(x=x,xend=xend,y=y,yend=yend), size = 2, color = 'black') +
  theme_publication(base_size = 24) +
  scale_x_discrete("Age/season", label = c('0/3','0/4','1/1','1/2','1/3','1/4','2/1','2/2','2/3','2/4')) +
  scale_y_continuous("Predicted length (mm)") +
  scale_color_manual(values = c('lightblue','red')) +
...
ggsave('./figures/predLensTempBnt.png', width = 11, height = 9, units = 'in')

# bnt flow
stats3Spp[['bnt']][[1]]$iter_flow <- paste(stats3Spp[['bnt']][[1]]$iter,stats3Spp[['bnt']][[1]]$flow, sep='_')
flowLoHiBnt <- stats3Spp[['bnt']][[1]] %>% filter( flow %in% c(-1.5,1.5), temp %in% 0, bkt %in% 0, bnt %in% 0, ats %in% 0)

ggplot(flowLoHiBnt, aes(occ,lenMean)) +
  geom_line(aes(color = factor(flow), group = iter_flow), alpha = 0.25) +
  geom_segment(data = segment_dataBNT, aes(x=x, xend=xend, y=y, yend=yend), size = 2, color = 'black') +
  theme_publication(base_size = 24) +
  scale_x_discrete("Age/season", label = c('0/3','0/4','1/1','1/2','1/3','1/4','2/1','2/2','2/3','2/4')) +
...
ggsave('./figures/predLensFlowBnt.png', width = 11, height = 9, units = 'in')


# bnt ATS
stats3Spp[['bnt']][[1]]$iter_ats <- paste(stats3Spp[['bnt']][[1]]$iter,stats3Spp[['bnt']][[1]]$ats, sep='_')

atsLoHiBNT <- stats3Spp[['bnt']][[1]] %>% filter( ats %in% c(-1.5,1.5), temp %in% 0, bkt %in% 0, bnt %in% 0, flow %in% 0)

ggplot(atsLoHiBNT, aes(occ,lenMean)) +
  geom_line(aes(color = factor(ats), group = iter_ats), alpha = 0.25) +
  geom_segment(data = segment_dataBNT, aes(x=x,xend=xend,y=y,yend=yend), size = 2, color = 'black') +
  theme_publication(base_size = 24) +
  scale_x_discrete("Age/season", label = c('0/3','0/4','1/1','1/2','1/3','1/4','2/1','2/2','2/3','2/4')) +
...
ggsave('./figures/predLensATSBnt.png', width = 11, height = 9, units = 'in')
...
bktLoHiBKT <- stats3Spp[['bkt']][[1]] %>% filter(iter == 1, bkt %in% c(-1.5,1.5), temp %in% 0, flow %in% 0, bnt %in% 0, ats %in% 0)

ggplot(bktLoHiBKT, aes(occ, lenMean, group = bkt)) +
  geom_line() +
  geom_segment(aes(x=x, xend=xend, y=y, yend=yend), size = 2, color = 'black', data = segment_dataBKT)

bntLoHi <- stats3Spp[['bnt']][[1]] %>% filter(iter == 1, bnt %in% c(-1.5,1.5), temp %in% 0, flow %in% 0, bkt %in% 0, ats %in% 0)

ggplot(bntLoHi, aes(occ,lenMean, group = bnt)) +
  geom_line() +
  geom_segment(x=4,xend=4,y=110,yend=135)

atsLoHi <- stats3Spp[['bkt']][[1]] %>% filter(iter == 1, ats %in% c(-1.5,1.5), temp %in% 0, flow %in% 0, bkt %in% 0, bnt %in% 0)

ggplot(atsLoHi, aes(occ,lenMean, color = factor(ats), group = ats)) +
  geom_line() +
  geom_segment(x=4,xend=4,y=110,yend=135)
